...
@php
$seo = App\Models\ProjectSeo::where('id', 1)->get();
@endphp
@section('title')
    ...
    <title>{{$seo->first()->title }}</title>
     ...
     <meta property="og:image" content="{{asset('upload/project_seo_img')}}/{{$seo->first()->image }}">
     <link rel="image_src" href="{{asset('upload/project_seo_img')}}/{{$seo->first()->image }}" />
@endsection
